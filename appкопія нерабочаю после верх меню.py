...
def image_to_data_uri(uploaded_file) -> Optional[str]:
    ...
    mime_type = uploaded_file.type.split('/')[1]
...
# ============================================================================
# ЕКСПОРТ / ІМПОРТ КОНФІГУРАЦІЇ (з збереженням у файл)
# ============================================================================
...
def save_config_to_file():
    """Зберігає конфігурацію у файл на диску"""
    try:
        config = {
...
def load_config_from_file():
    """Завантажує конфігурацію з файлу при запуску"""
...
def export_config():
    """Експортує конфігурацію у JSON для завантаження"""
    config = {
        'name': st.session_state.get('name_value', ''),
        'bio': st.session_state.get('bio_value', ''),
        'theme_color': st.session_state.get('theme_color_value', '#667eea'),
        ...
        'contact_info': st.session_state.get('contact_info', ''),
        'products': st.session_state.get('products', []),
                'enable_navbar': st.session_state.get('enable_navbar', True),
        'navbar_style': st.session_state.get('navbar_style', 'transparent'),
        'navbar_position': st.session_state.get('navbar_position', 'top'),
        'scroll_to_top': st.session_state.get('scroll_to_top', True),
        'show_progress_bar': st.session_state.get('show_progress_bar', True),
    }
    return json.dumps(config, ensure_ascii=False, indent=2)

def import_config(json_string):
    """Імпортує конфігурацію з JSON і зберігає у файл"""
    try:
        ...
        st.session_state.contact_info = config.get('contact_info', '')
        st.session_state.products = config.get('products', [])
        st.session_state['enable_navbar'] = config.get('enable_navbar', True)
        st.session_state['navbar_style'] = config.get('navbar_style', 'transparent')
        st.session_state['navbar_position'] = config.get('navbar_position', 'top')
        st.session_state['scroll_to_top'] = config.get('scroll_to_top', True)
        st.session_state['show_progress_bar'] = config.get('show_progress_bar', True)
        
        # Зберігаємо у файл для відновлення після F5
        save_config_to_file()
        return True
    except Exception as e:
...
# Завантажуємо конфігурацію при запуску
...
def generate_navbar(name: str, blocks: List[str], theme_color: str, theme_styles: Dict, 
                    navbar_style: str, navbar_position: str, scroll_to_top: bool, 
                    show_progress_bar: bool) -> str:
    """Генерує навігаційну панель"""
    if not blocks:
        return ""
    
    # Генеруємо пункти меню
    nav_items = []
    block_labels = {
        'links': ('🔗', 'Посилання'),
        'gif': ('🎞️', 'GIF'),
    ...
    for block in blocks:
        if block in block_labels:
            icon, label = block_labels[block]
            nav_items.append(f'<a href="#section-{block}" class="nav-link" data-section="section-{block}">{icon} {label}</a>')
    ...
    if navbar_style == 'transparent':
        ...
        panel_border = '1px solid rgba(255, 255, 255, 0.2)'
    elif navbar_style == 'solid':
        panel_bg = theme_styles['container_bg']
        panel_border = 'none'
    else:  # blur
    ...
    if navbar_position == 'top':
        position_css = "top: 0;"
    elif navbar_position == 'bottom':
        position_css = "bottom: 0;"
    else:  # both - тільки верхня
        position_css = "top: 0;"
    
    # Прогрес-бар
    progress_html = ""
    if show_progress_bar:
        progress_html = f'<div class="scroll-progress" id="scrollProgress" style="background: {theme_color};"></div>'
    
    # Кнопка "Вгору"
    scroll_top_html = ""
    if scroll_to_top:
        scroll_top_html = f'<button class="scroll-to-top" id="scrollToTop" onclick="scrollToTop()" aria-label="Повернутися вгору">⬆️</button>'
    
    return f"""
    {progress_html}
    <nav class="navbar" id="navbar" style="{position_css} background: {panel_bg}; border: {panel_border};">
        <div class="navbar-container">
            <a href="#top" class="navbar-brand">{name}</a>
            <button class="navbar-toggle" id="navbarToggle" aria-label="Меню">
...
def generate_products_block(products: List[Dict], theme_color: str) -> str:
    ...
    for prod in products:
        if prod.get('image'):
            safe_title = prod['title'].replace("'", "\\'")
            img_html = f"""
            <div class="product-image-wrapper" onclick="openProductLightbox('{prod["image"]}', '{safe_title}')">
        ...
        <div class="product-card">
            ...
            <button class="product-btn" onclick="buyProduct('{prod['title']}', {prod.get('price', 0)}, '{prod.get('buy_link', '#')}')">🛒 Купити</button>
        </div>"""
    return f"""
    ...
    <div class="modal" id="productModal">
        <div class="modal-content">
            ...
            <h2 class="modal-title" id="productModalTitle">Товар</h2>
            <div class="modal-price" id="productModalPrice" style="color: {theme_color};">$0</div>
            <p class="modal-description">Натисніть "Перейти до покупки", щоб оформити замовлення</p>
            <div class="modal-buttons">
                <button class="btn-pay" onclick="confirmProductPurchase()">💳 Перейти до покупки</button>
                <button class="btn-cancel" onclick="closeProductModal()">Скасувати</button>
            </div>
        </div>
    </div>"""
...
def generate_full_html(name, bio, avatar_image_data_uri, links, theme_color, theme_choice, font_choice, dark_mode, background_image_data_uri, faq_items, countdown_date, countdown_title, custom_html, gif_url, gif_caption, quote_text, quote_author, features, gallery_images, contact_title, contact_info, products):
    ...
    links_html = "".join([f"""<a href="#" class="link {'paid' if link.get('is_paid') else ''}" id="{link['id']}" 
        ...
        {link['title']}{'<span class="price-badge">$' + str(link.get('price', 0)) + '</span>' if link.get('is_paid') else ''}
           ...
           # Спочатку створюємо extra_blocks
    ...
    # Визначаємо наявні блоки для навігації
    active_blocks = []
    if links: active_blocks.append('links')
    if gif_url: active_blocks.append('gif')
    if quote_text: active_blocks.append('quote')
    if features: active_blocks.append('features')
    if gallery_images: active_blocks.append('gallery')
    if products: active_blocks.append('products')
    if faq_items: active_blocks.append('faq')
    if countdown_date: active_blocks.append('countdown')
    if contact_title: active_blocks.append('contact')
    
    # Тепер додаємо id до секцій (extra_blocks вже існує!)
    if gif_url:
        extra_blocks = extra_blocks.replace('<div class="gif-section">', '<div class="gif-section" id="section-gif">')
    if quote_text:
        extra_blocks = extra_blocks.replace('<div class="quote-section">', '<div class="quote-section" id="section-quote">')
    if features:
        extra_blocks = extra_blocks.replace('<div class="features-section">', '<div class="features-section" id="section-features">')
    if gallery_images:
        extra_blocks = extra_blocks.replace('<div class="gallery-section">', '<div class="gallery-section" id="section-gallery">')
    if products:
        extra_blocks = extra_blocks.replace('<div class="products-section">', '<div class="products-section" id="section-products">')
    if faq_items:
        extra_blocks = extra_blocks.replace('<div class="faq-section">', '<div class="faq-section" id="section-faq">')
    if countdown_date:
        extra_blocks = extra_blocks.replace('<div class="countdown-section">', '<div class="countdown-section" id="section-countdown">')
    if contact_title:
        extra_blocks = extra_blocks.replace('<div class="contact-section">', '<div class="contact-section" id="section-contact">')
    
    # Генеруємо навігацію
    navbar_html = ""
    if st.session_state.get('enable_navbar', True):
        navbar_html = generate_navbar(
            name=name,
...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        body {{ font-family: '{font_choice}', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; {background_css} min-height: 100vh; display: flex; justify-content: center; align-items: center; padding: 20px; position: relative; transition: background 0.3s ease; }}
        ...
        #heatmap-container {{ position: fixed; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none; z-index: 9999; }}
        .heatmap-toggle {{ position: fixed; bottom: 20px; right: 20px; background: #333; color: white; border: none; padding: 10px 20px; border-radius: 25px; cursor: pointer; z-index: 10000; font-size: 14px; }}
...
<body data-dark-mode="{str(dark_mode).lower()}">
    ...
    <button class="heatmap-toggle" onclick="toggleHeatmap()">🔥 Heatmap</button>
    ...
    <script>
        ...
        function openProductLightbox(imageSrc, title) {{
            document.getElementById('productImageModalImg').src = imageSrc;
        ...
        function toggleHeatmap() {{
            ...
            if (container.style.display === 'none') {{ container.style.display = 'block'; panel.classList.add('show'); updateStats(); }}
            else {{ container.style.display = 'none'; panel.classList.remove('show'); }}
        }}
        ...
        window.addEventListener('scroll', () => {{
            ...
            if (navbar && navbar.classList.contains('hidden') === false) {{
...
st.title("🚀 OLDO site builder")
st.subheader("Створи свій сайт-візитку за 5 хвилин")
...
with tab1:
    ...
    name = st.text_input("Твоє ім'я або назва бренду", value=st.session_state.get('name_value', ''), placeholder="Наприклад: Іван Петренко", key="name_input")
    if name: 
    ...
    with col1:
        bio = st.text_area("Короткий опис про себе", value=st.session_state.get('bio_value', ''), placeholder="Наприклад: Фрілансер-дизайнер з Києва.", key="bio_input")
    with col2:
        st.write(""); st.write("")
    ...
    if avatar_option == "Завантажити файл":
        ...
        if avatar_image_data_uri: st.success("✅ Аватар завантажено!")
    else:
        avatar_url = st.text_input("Посилання на аватар", placeholder="https://example.com/avatar.jpg")
        if avatar_url: avatar_image_data_uri = avatar_url
    if avatar_image_data_uri: progress_items += 1

    st.subheader("🎨 Обери стиль")
    col1, col2 = st.columns(2)
    with col1:
        theme_color = st.color_picker("Основний колір", value=st.session_state.get('theme_color_value', '#667eea'), key="color_picker")
        st.session_state['theme_color_value'] = theme_color
    with col2:
        theme_choice = st.radio("Тема дизайну", list(THEMES.keys()), index=list(THEMES.keys()).index(st.session_state.get('theme_choice_value', 'gradient')) if st.session_state.get('theme_choice_value', 'gradient') in THEMES else 0, format_func=lambda x: THEMES[x]['name'], key="theme_radio")
    ...
    with col3:
        font_choice = st.selectbox("Шрифт", FONTS, index=FONTS.index(st.session_state.get('font_choice_value', 'Inter')) if st.session_state.get('font_choice_value', 'Inter') in FONTS else 0, key="font_select")
    ...
    with col4:
        dark_mode = st.checkbox("🌙 Dark mode за замовчуванням", value=st.session_state.get('dark_mode_value', False), key="dark_mode_check")
    ...
    st.subheader("🧭 Навігаційна панель")
    
    enable_navbar = st.checkbox("✨ Увімкнути навігаційну панель", value=True, key="enable_navbar")
    
    if enable_navbar:
        col_nav1, col_nav2, col_nav3 = st.columns(3)
        with col_nav1:
            navbar_style = st.selectbox(
                "Стиль панелі",
                ["transparent", "solid", "blur"],
                format_func=lambda x: {"transparent": "🔍 Прозора", "solid": "🎨 Кольорова", "blur": "💫 З розмиттям"}[x],
                key="navbar_style"
            )
        with col_nav2:
            navbar_position = st.selectbox(
                "Позиція",
                ["top", "bottom", "both"],
                format_func=lambda x: {"top": "⬆️ Зверху", "bottom": "⬇️ Знизу", "both": "↕️ Обидві"}[x],
                key="navbar_position"
            )
        with col_nav3:
            scroll_to_top = st.checkbox("⬆️ Кнопка 'Вгору'", value=True, key="scroll_to_top")
        
        show_progress_bar = st.checkbox("📊 Прогрес-бар скролу", value=True, key="show_progress_bar")
    ...
    with col1:
        ...
        st.download_button(
            ...
            data=config_json,
            file_name=f"oldo_config_{name.replace(' ', '_') if name else 'project'}.json",
    ...
    with col_c2:
        if st.button("🗑️ Очистити всі дані та почати знову", type="secondary", use_container_width=True, key="clear_all_btn"):
            ...
            keys_to_clear = [
                'name_value', 'bio_value', 'theme_color_value', 'theme_choice_value', 
                'font_choice_value', 'dark_mode_value', 'links_list', 'faq_items', 
                'countdown_date', 'countdown_title', 'custom_html', 'gif_url', 
                'gif_caption', 'quote_text', 'quote_author', 'features', 
                'gallery_images', 'contact_title', 'contact_info', 'products',
                'generated_bio', 'config_loaded'
            ]
            for key in keys_to_clear:
                if key in st.session_state:
                    del st.session_state[key]
            st.success("✅ Всі дані очищено!")
            st.rerun()
