...
from datetime import datetime, timedelta  # <-- ПЕРЕМІЩЕНО НА ПОЧАТОК
...
def register_user(email: str, password: str, full_name: str) -> Dict[str, Any]:
    try:
        ...
        if response.user:
            return {"success": True, "user_id": response.user.id, "email": response.user.email, "error": None}
        else:
            return {"success": False, "user_id": None, "error": "Не вдалося зареєструвати користувача"}
...
def login_user(email: str, password: str) -> Dict[str, Any]:
    try:
        ...
        if response.user:
            ...
            return {"success": False, "user_id": None, "error": "Невірний email або пароль"}
    except Exception as e:
        return {"success": False, "user_id": None, "error": str(e)}
        ...
        if links:
            ...
            for idx, link in enumerate(links):
                links_to_insert.append({
                    'profile_id': user_id, 'title': link.get('title', ''), 'url': link.get('url', ''),
                    'is_paid': link.get('is_paid', False), 'price': link.get('price', 0), 'position': idx
                })
        ...
        if products:
            ...
            for idx, prod in enumerate(products):
                products_to_insert.append({
                    'profile_id': user_id, 'title': prod.get('title', ''), 'description': prod.get('description', ''),
                    'price': prod.get('price', 0), 'image_url': prod.get('image', ''), 'is_available': True, 'position': idx
                })
...
def send_order_notification(profile_id: str, order_id: str, order_data: Dict[str, Any]):
    try:
        ...
        items_text = "\n".join([f"• {html.escape(str(item.get('title', '')))} — {item.get('price', 0)}₴ × {item.get('quantity', 1)}" for item in order_data.get('items_json', [])])
...
💬 Коментар: {comment}"""
        ...
        payload = {"chat_id": str(chat_id), "text": message, "parse_mode": "HTML"}
...
def track_event(profile_id: str, event_type: str, target_id: Optional[str] = None):
    try:
        supabase.table('analytics').insert({'profile_id': profile_id, 'event_type': event_type, 'target_id': target_id}).execute()
    except Exception as e:
...
def activate_pro_trial(user_id: str) -> bool:
    """Активує 14-денний безкоштовний період Pro."""
    try:
        ...
        print(f"Помилка активації триалу: {e}")
        return False