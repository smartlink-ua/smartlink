...
def generate_products_block(products: List[Dict], theme_color: str) -> str:
    """Генерує каталог товарів з робочим lightbox для фото"""
    ...
    for idx, prod in enumerate(products):
        ...
        <div class="product-card">
            ...
            <button class="product-btn" onclick="buyProduct('{safe_title_btn}', {prod.get('price', 0)}, '{prod.get('buy_link', '#')}')">🛒 Купити</button>
        </div>"""
    ...
    <script>
        function openProductLightbox(element) {{
            const src = element.getAttribute('data-img-src');
            const title = element.getAttribute('data-img-title');
...
with tab1:
    ...
    if avatar_option == "Завантажити файл":
        ...
        default_url = '' if saved_avatar.startswith('data:') else saved_avatar
